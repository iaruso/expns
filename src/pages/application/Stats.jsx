    ...
    const filterTransactions = () => {
      ...
      filteredTransactions.forEach(transaction => {
        if (categoryMap.has(transaction.category)) {
          ...
          categoryMap.set(transaction.category.charAt(0).toUpperCase() + transaction.category.slice(1), convertCurrency(transaction.amount, transaction.currency, localCurrency, currencyRates));
        }
      });
      ...
      <div className='stats-content flex flex-col flex-1 overflow-y-auto bg-alabaster rounded relative'>
        <div className='absolute right-2 top-2 h-8 bg-white rounded flex items-center p-1 gap-1 z-10'>
          {['day', 'week', 'month', 'year'].map(timeFrame => (
            <button
              key={timeFrame}
              className={`h-6 flex px-2 py-1 text-xs font-semibold text-gray items-center justify-center rounded border-[0.05rem] mobile:border-[0.1rem] border-gallery bg-white hover:bg-alabaster hover:duration-[0.4s] ease-in-out ${selectedTimeFrame === timeFrame ? 'bg-alabaster' : 'bg-white'}`}

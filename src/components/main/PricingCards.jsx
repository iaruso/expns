...
const PricingPlanCard = ({ titlePath, subtitlePath, featuresPath, ctaPath, ctaBgColor, ctaBgHover }) => {
  ...
  return (
    <div className='flex flex-col gap-2 p-4 w-full h-full rounded-lg border-[0.05rem] mobile:border-[0.1rem] border-gallery'>
      ...
      <p className='two-lines h-[2.5rem] mobile:h-auto text-tiny mobile:text-base text-cod font-semibold'>{t(subtitlePath)}</p>
      <ul className='text-sm mobile:text-tiny text-shaft font-semibold list-disc list-inside p-2 gap-2 flex-1'>
        {featuresPath.map((featurePath, index) => (
          <li key={index}>{t(featurePath)}</li>	
        ))}
      </ul>
      <button className={`w-full h-8 mobile:h-10 text-sm mobile:text-base text-white font-semibold flex justify-center items-center rounded py-1 px-2 cursor-pointer bg-${ctaBgColor} hover:bg-${ctaBgHover} hover:duration-[0.4s] ease-in-out`} rel='nofollow'>{t(ctaPath)}</button>
    </div>
  );
}

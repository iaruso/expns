...
const PricingPlanCard = ({ titlePath, subtitlePath, featuresPath, ctaPath, ctaBgColor, ctaBgHover }) => {
  return (
    <div className='flex flex-col gap-2 p-4 w-full h-full rounded-lg border-[0.05rem] mobile:border-[0.1rem] border-gallery'>
      ...
      <button className={`w-full h-8 mobile:h-10 text-sm mobile:text-base text-white font-semibold flex justify-center items-center rounded py-1 px-2 cursor-pointer bg-${ctaBgColor} hover:bg-${ctaBgHover} duration-200`} rel='nofollow'><Translator path={ctaPath}/></button>
    </div>
  );
}
